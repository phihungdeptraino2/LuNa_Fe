...
import { Eye, ArrowLeft, X, RefreshCw } from "lucide-react";
...
const AdminOrderManager = () => {
  ...
  const handleViewDetail = async (id) => {
    try {
      ...
      if (data) {
        ...
        setUpdatingStatus(data.status); // Set default cho select box
      }
    } catch (error) {
  ...
  const handleUpdateStatus = async () => {
    ...
    if (updatingStatus === selectedOrder.status)
      return alert("Vui lòng chọn trạng thái khác!");

    const confirmUpdate = window.confirm(
      `Bạn có chắc chắn muốn chuyển đơn hàng sang trạng thái: ${updatingStatus}?`
    );
    if (!confirmUpdate) return;

    try {
      // Gọi qua Service
      await updateOrderStatus(selectedOrder.id, updatingStatus);

      alert("Cập nhật trạng thái thành công!");

      // Tắt Modal và Load lại danh sách
  ...
  const getUserInfo = (order) => {
    ...
    // Xử lý nhiều trường hợp response từ backend
    const name =
      order.userName ||
      order.user?.fullname ||
      order.userResponseDTO?.fullname ||
      "Khách lẻ";
  ...
  const getAddressString = (order) => {
    ...
    // Ưu tiên các trường phẳng (flat fields) nếu có
    ...
    // Fallback nếu address là object
          ...
          <div style={styles.modalContent}>
            ...
            <div style={styles.modalBody}>
              ...
              <div style={styles.infoGrid}>
                <p>
                  <b>Tài khoản:</b> {getUserInfo(selectedOrder).name}
                </p>
                <p>
                  <b>Địa chỉ:</b> {getAddressString(selectedOrder)}
                </p>
                <p>
                  <b>Ngày đặt:</b> {formatDate(selectedOrder.orderDate)}
                </p>
                <p>
              ...
              <div style={styles.totalSection}>
                Tổng cộng:{" "}
                <span style={{ fontSize: 20, color: "#d32f2f" }}>
                  {formatCurrency(
